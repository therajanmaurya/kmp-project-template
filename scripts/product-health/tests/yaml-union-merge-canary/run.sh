...
# added later (core_store.stores / cache_keys / packages, database.daos / type_converters) was taken
# from the TEMPLATE wholesale, so a fork that declared its own store, cache key, DAO or package lost
# it on the next sync. Silently: the generated Kotlin then regenerated faithfully without the row,
...
#   U-4 identity is per-list: id · name|fn · name · whole-value
...
database:
  daos:
    - {name: sharedDao, type: kpt.core.database.SharedDao}
  type_converters:
    - kpt.core.database.SharedConverters
network:
  access_points:
    - id: main
      owner: fork
...
database:
  daos:
    - {name: sharedDao, type: kpt.core.database.SharedDao}
    - {name: forkDao, type: kpt.core.database.ForkDao}
  type_converters:
    - kpt.core.database.SharedConverters
    - kpt.core.database.ForkConverters
network:
  access_points:
    - id: main
      owner: fork
...
database:
  daos:
    - {name: sharedDao, type: kpt.core.database.SharedDao}
    - {name: newDemoDao, type: kpt.core.database.NewDao}
  type_converters:
    - kpt.core.database.SharedConverters
    - kpt.core.database.NewConverters
network:
  access_points:
    - id: main
      owner: fork
...
u1=0
for tok in forkStore FORK_KEY forkOf forkpkg forkDao ForkConverters forkApi; do
  grep -q "$tok" "$OUT" || { bad "U-1 fork row '$tok' was DROPPED by the merge — a fork loses its own declaration on sync"; u1=1; }
done
[ "$u1" -eq 0 ] && ok "U-1 every fork row survived (7 rows across 6 lists)"

# U-2 — template-only rows still arrive
u2=0
for tok in newDemo NEW_DEMO newpkg newDemoDao NewConverters newDemoApi; do
...
for pair in "id: shared,|1" "name: SHARED,|1" "id: sharedpkg,|1" "name: sharedDao,|1" "SharedConverters$|1" "id: main$|1"; do
  tok="${pair%|*}"; want="${pair##*|}"
  got=$(grep -cE "$tok" "$OUT")
