# `frankfurter` — access point package

SCAFFOLDED by `./gradlew syncForkConfig` from the `frankfurter` access point in
...
`type: rest` — the binding is `restApi("frankfurter") { it.create${simple}Api() }`, so Ktorfit
