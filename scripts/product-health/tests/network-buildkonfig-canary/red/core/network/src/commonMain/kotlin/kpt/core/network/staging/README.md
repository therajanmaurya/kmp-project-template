# `staging` — access point package

SCAFFOLDED by `./gradlew syncForkConfig` from the `staging` access point in
...
`type: rest` — the binding is `restApi("staging") { it.create${simple}Api() }`, so Ktorfit
