# `fred` — access point package

SCAFFOLDED by `./gradlew syncForkConfig` from the `fred` access point in
...
`type: rest` — the binding is `restApi("fred") { it.create${simple}Api() }`, so Ktorfit
