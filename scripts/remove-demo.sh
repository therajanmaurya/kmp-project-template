...
# ── 4a2. Reset the generated STORE surfaces ───────────────────────────────────────────
#     Same hazard as GeneratedApiBindings above. Both files are derived from
#     `app-profile#core_store.stores`, whose demo rows step 3 has already stripped — but the OUTPUT
#     is committed source, so without this a cleaned fork keeps 20 bindings referencing
#     `provideAlertsStore` and friends that step 4b2 is about to delete (unresolved references), plus
#     qualifiers for stores it no longer ships. `syncForkConfig` rewrites both from whatever rows
#     survive; this reset is what makes the tree compile in between.
STORE_REG="core/store/src/commonMain/kotlin/kpt/core/store/config/AppStoreRegistry.kt"
STORE_BIND="core/store/src/commonMain/kotlin/kpt/core/store/di/GeneratedStoreBindings.kt"
echo "reset generated store surfaces:"
if [ -f "$STORE_REG" ]; then
  say "reset ${STORE_REG#./}"
  if [ "$APPLY" -eq 1 ]; then
    cat > "$STORE_REG" <<'STOREREGEOF'
/*
 * Copyright 2026 Mifos Initiative
 *
 * This Source Code Form is subject to the terms of the Mozilla Public
...
object AppStoreRegistry : StoreRegistry() {
    // No store declared yet — add one in app-profile/app.yaml#core_store.stores.

    object Ttl

    val ttlByName: Map<String, Duration> = emptyMap()
}
STOREREGEOF
  fi
fi
if [ -f "$STORE_BIND" ]; then
  say "reset ${STORE_BIND#./}"
  if [ "$APPLY" -eq 1 ]; then
    cat > "$STORE_BIND" <<'STOREBINDEOF'
/*
 * Copyright 2026 Mifos Initiative
 *
 * This Source Code Form is subject to the terms of the Mozilla Public
