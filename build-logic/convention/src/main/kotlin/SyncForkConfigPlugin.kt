...
abstract class SyncForkConfigTask : DefaultTask() {
    ...
    fun sync() {
        ...
        if (hasAppProfile) {
            ...
            regenerateDaoBindings(root, appProfile)
            regenerateStoreKeys(root, appProfile)
            regenerateStoreBindings(root, appProfile)
        }
  ...
  "visibility": "$msVisibility"
     ...
     */
    /**
     * Regenerate `GeneratedDaoBindings.kt` — one Koin binding per DAO declared in
     * `app-profile/app.yaml#database.daos`.
     *
     * Every binding is mechanically `single { get<AppDatabase>().<name> }`, so it is DERIVABLE from
     * the same declaration that puts the accessor on AppDatabase. They used to be hand-written in a
     * `DemoDatabaseModule`, which meant a declared DAO silently had no binding until someone
     * remembered to add one — the database twin of the wiring step `GeneratedApiBindings` removed
     * for network.
     *
     * `owner: fork` and NOT under a domain package: it is derived from the FORK's app-profile, and
     * `remove-demo.sh` deletes domain packages outright, which would take the codegen target with it.
     */
    private fun regenerateDaoBindings(root: File, appProfile: Map<String, Any?>) {
        val dir = File(root, "core/database/src/commonMain/kotlin/kpt/core/database/di")
        if (!dir.isDirectory) return
        val db = (appProfile["database"] as? Map<*, *>).orEmpty()
        val daos = ((db["daos"] as? List<*>) ?: emptyList<Any?>()).mapNotNull { row ->
            (row as? Map<*, *>)?.get("name")?.toString()?.trim()?.takeIf { it.isNotEmpty() }
        }
        val sb = StringBuilder()
        sb.append("/*\n")
        sb.append(" * Copyright 2026 Mifos Initiative\n")
        ...
        sb.append(" * See See https://github.com/openMF/kmp-project-template/blob/main/LICENSE\n")
        sb.append(" */\n")
        sb.append("package kpt.core.database.di\n\n")
        sb.append("import kpt.core.database.AppDatabase\n")
        sb.append("import org.koin.core.module.Module\n")
        sb.append("import org.koin.dsl.module\n\n")
        sb.append("/**\n")
        sb.append(" * GENERATED by `./gradlew syncForkConfig` from `app-profile/app.yaml#database.daos`\n")
        sb.append(" * — one Koin binding per declared DAO. DO NOT HAND-EDIT.\n")
        sb.append(" *\n")
        sb.append(" * To add a DAO: declare it in app-profile (which also puts the accessor on AppDatabase)\n")
        sb.append(" * and re-run syncForkConfig. There is no wiring step — that is the point.\n")
        sb.append(" *\n")
        sb.append(" * Pulled in by `DatabaseModule` via `includes(GeneratedDaoBindings)`.\n")
        sb.append(" */\n")
        sb.append("val GeneratedDaoBindings: Module = module {\n")
        if (daos.isEmpty()) {
            sb.append("    // No DAO declared yet — add one in app-profile/app.yaml#database.daos.\n")
        } else {
            daos.forEach { sb.append("    single { get<AppDatabase>().").append(it).append(" }\n") }
        }
        sb.append("}\n")
        val file = File(dir, "GeneratedDaoBindings.kt")
        val next = sb.toString()
        if (!file.isFile || file.readText() != next) {
            file.writeText(next)
            logger.lifecycle("syncForkConfig: regenerated GeneratedDaoBindings (${daos.size} DAO(s))")
        }
    }

    /** Rows of `app-profile/app.yaml#core_store.stores`, in declaration order. */
    ...
    /**
     * One `<Store>Keys` object per declared store, written INTO THAT STORE'S OWN PACKAGE — the
     * store's complete addressing surface in one place: its Koin qualifier, its TTL, and its cache
     * keys.
     *
     * This replaced a single `config/AppStoreRegistry.kt` holding every qualifier. Two things made
     * that the wrong shape. A demo store's qualifier lived in a file that SURVIVES `remove-demo.sh`
     * and was only emptied by a later regeneration, so the strip was not structural; and
     * `AppStoreRegistry.Ttl.COIN_MARKETS` was a same-package round trip through a global — the
     * crypto store factory reaching across the module for its own store's TTL.
     *
     * `StoreRegistry` is a thin qualifier factory (`protected fun store(name) = named(name)`) with no
     * shared state, so N objects behave exactly as one did: `named("loans")` is the same qualifier
     * wherever it is declared.
     *
     * The old `ttlByName` runtime map is NOT reproduced. It had zero consumers — only the generator
     * that wrote it — and advertised a lookup-by-name use case nothing exercised.
     */
    private fun regenerateStoreKeys(root: File, appProfile: Map<String, Any?>) {
        val storeRoot = File(root, "core/store/src/commonMain/kotlin/kpt/core/store")
        if (!storeRoot.isDirectory) return
        var written = 0

        for (row in declaredStores(appProfile)) {
            val provider = storeStr(row, "provider")
            if (provider.isEmpty()) continue
            // provider FQN carries the package: kpt.core.store.banking.impl.provideLoansStore
            val pkg = provider.substringBeforeLast('.').removeSuffix(".impl")
            val dirName = pkg.substringAfterLast('.')
            val dir = File(storeRoot, dirName)
            if (!dir.isDirectory) {
                logger.lifecycle("syncForkConfig: skipping keys for '${storeStr(row, "id")}' — no package $dirName")
                continue
            }
            val qualifier = storeStr(row, "qualifier")
            val objName = qualifier + "Keys"
            val ttl = storeStr(row, "ttl")
            val keys = ((row["cache_keys"] as? List<*>) ?: emptyList<Any?>()).mapNotNull { it as? Map<*, *> }
            ...
            if (ttl.isNotEmpty()) {
                sb.append("import kotlin.time.Duration.Companion.")
                  .append(when { ttl.endsWith("d") -> "days"; ttl.endsWith("h") -> "hours"; else -> "minutes" })
                  .append("\n")
            }
            sb.append("\n/**\n")
            sb.append(" * GENERATED by `./gradlew syncForkConfig` from this store's row in\n")
            sb.append(" * `app-profile/app.yaml#core_store.stores`. DO NOT HAND-EDIT.\n")
            sb.append(" *\n")
            sb.append(" * Everything that ADDRESSES the `").append(storeStr(row, "id")).append("` store: its Koin\n")
            sb.append(" * qualifier")
            if (ttl.isNotEmpty()) sb.append(", its freshness window")
            if (keys.isNotEmpty()) sb.append(", and its stream cache keys")
            sb.append(".\n")
            sb.append(" *\n")
            sb.append(" * Lives in the store's own package so it shares the store's lifecycle — strip the store and\n")
            sb.append(" * this goes with it, with no second declaration to keep in sync.\n")
            sb.append(" */\n")
            sb.append("object ").append(objName).append(" : StoreRegistry() {\n")
            sb.append("    /** Koin qualifier — `single(").append(objName).append(".Qualifier) { … }`. */\n")
            sb.append("    val Qualifier = store(\"").append(storeStr(row, "id")).append("\")\n")
            if (ttl.isNotEmpty()) {
                sb.append("\n    /** Freshness window, declared as `ttl:` in app-profile. */\n")
                sb.append("    val TTL = ").append(ttlExpression(ttl)).append("\n")
            }
            val consts = keys.filter { storeStr(it, "name").isNotEmpty() }
            val builders = keys.filter { storeStr(it, "fn").isNotEmpty() }
            if (consts.isNotEmpty() || builders.isNotEmpty()) sb.append("\n")
            consts.forEach { k ->
                sb.append("    const val ").append(storeStr(k, "name"))
                  .append(" = \"").append(storeStr(k, "key")).append("\"\n")
            }
            if (consts.isNotEmpty() && builders.isNotEmpty()) sb.append("\n")
            builders.forEach { k ->
                val params = ((k["params"] as? List<*>) ?: emptyList<Any?>()).mapNotNull { it as? Map<*, *> }
                val sig = params.joinToString(", ") { "${storeStr(it, "name")}: ${storeStr(it, "type")}" }
                sb.append("    fun ").append(storeStr(k, "fn")).append("(").append(sig).append("): String = \"")
                  .append(interpolate(storeStr(k, "key"), params.map { storeStr(it, "name") }))
                  .append("\"\n")
            }
            sb.append("}\n")

            val file = File(dir, "$objName.kt")
            val next = sb.toString()
            if (!file.isFile || file.readText() != next) { file.writeText(next); written++ }
        }
        if (written > 0) logger.lifecycle("syncForkConfig: regenerated $written per-store Keys object(s)")
    }

    /**
     * `"currency:rateHistory:{from}-{to}-{days}d"` -> `currency:rateHistory:$from-$to-${days}d`.
     *
     * Braces are emitted when the next character could continue an identifier, because `"$days" + "d"`
     * would otherwise parse as a reference to a variable named `daysd`. Getting that wrong produces
     * a compile error rather than a silent bad key, but only for SOME rows — hence the explicit rule.
     */
    private fun interpolate(key: String, paramNames: List<String>): String {
        var out = key
        paramNames.forEach { name ->
            val idx = out.indexOf("{$name}")
            if (idx < 0) error("cache_keys: '$key' has no {$name} placeholder for its declared param")
            val after = out.getOrNull(idx + name.length + 2)
            val needsBraces = after != null && (after.isLetterOrDigit() || after == '_')
            out = out.replace("{$name}", if (needsBraces) "\${$name}" else "\$$name")
        }
        // Negative lookbehind: `${days}` is what THIS function just emitted, not an unreplaced
        // placeholder. Without it the check fires on its own output for every braced param.
        Regex("(?<!\\$)\\{([A-Za-z0-9_]+)}").find(out)?.let {
            error("cache_keys: '$key' has an undeclared placeholder {${it.groupValues[1]}}")
        }
        return out
    }

    /**
     * `core/store/**/di/GeneratedStoreBindings.kt` — one Koin binding per declared store PLUS the
     * StoreCacheManager logout registration, from the same rows.
     */
    private fun regenerateStoreBindings(root: File, appProfile: Map<String, Any?>) {
        val dir = File(root, "core/store/src/commonMain/kotlin/kpt/core/store/di")
        if (!dir.isDirectory) return
        val stores = declaredStores(appProfile)
        val purged = stores.filter { it["logout"] == true }

        val sb = StringBuilder()
        licenseHeader(sb)
        sb.append("package kpt.core.store.di\n\n")
        // ktlint's import-ordering rule is lexicographic, so every import is collected and sorted
        // together rather than appended in logical groups: `kpt.core.store.alerts…` sorts BEFORE
        // `kpt.core.store.config.AppStoreRegistry`, and emitting them in group order would make
        // spotless rewrite the file on every build, fighting the generator forever.
        // Each store contributes TWO imports: its provider function and its <Store>Keys object,
        // both from the store's own package.
        fun keysFqn(row: Map<*, *>): String {
            val prov = storeStr(row, "provider")
            val pkg = prov.substringBeforeLast('.').removeSuffix(".impl")
            return "$pkg.${storeStr(row, "qualifier")}Keys"
        }
        val imports = buildList {
            add("kpt.core.base.store.infra.StoreCacheManager")
            add("kpt.core.base.store.infra.impl.StoreCacheManagerImpl")
            addAll(stores.map { storeStr(it, "provider") }.filter { it.isNotEmpty() })
            addAll(stores.filter { storeStr(it, "provider").isNotEmpty() }.map { keysFqn(it) })
        ...
        sb.append("\n")
        sb.append("/**\n")
        sb.append(" * GENERATED by `./gradlew syncForkConfig` from `app-profile/app.yaml#core_store.stores`.\n")
        sb.append(" * DO NOT HAND-EDIT — declare the store in app-profile and re-run syncForkConfig.\n")
        sb.append(" *\n")
        sb.append(" * Carries BOTH the binding and the logout registration, because they were two hand-kept\n")
        sb.append(" * lists that had to agree: a store bound but never registered survives sign-out and shows\n")
        sb.append(" * the previous user's cached rows to the next one on a shared device. Generating both from\n")
        sb.append(" * one `logout:` field makes that drift impossible rather than merely detected by LP-1.\n")
        sb.append(" *\n")
        sb.append(" * `*Mutable` stores declare `logout: false`: Store5 5.1's MutableStore is not a `Store`\n")
        sb.append(" * subtype so `register` cannot accept one. Their rows still go — each shares a Room table\n")
        sb.append(" * with its read store, whose deleteAll wipes it.\n")
        sb.append(" *\n")
        sb.append(" * Pulled in by `StoreModule` via `includes(GeneratedStoreBindings)`.\n")
        sb.append(" */\n")
        sb.append("val GeneratedStoreBindings: Module = module {\n")
        if (stores.isEmpty()) {
            sb.append("    // No store declared yet — add one in app-profile/app.yaml#core_store.stores.\n")
        } else {
            stores.forEach { row ->
                val fn = storeStr(row, "provider").substringAfterLast('.')
                val deps = ((row["deps"] as? List<*>) ?: emptyList<Any?>()).map { it.toString().trim() }
                val args = deps.joinToString(", ") { "$it = get()" }
                sb.append("    single(").append(storeStr(row, "qualifier")).append("Keys.Qualifier) { ")
                  .append(fn).append("(").append(args).append(") }\n")
            }
            if (purged.isNotEmpty()) {
                sb.append("\n")
                sb.append("    // Logout purge — every `logout: true` store, registered at start so the manager\n")
                sb.append("    // holds them before the first sign-out.\n")
                sb.append("    single(createdAtStart = true) {\n")
                sb.append("        val mgr = get<StoreCacheManager>() as StoreCacheManagerImpl\n")
                purged.forEach { row ->
                    sb.append("        mgr.register(get(")
                      .append(storeStr(row, "qualifier")).append("Keys.Qualifier))\n")
                }
                sb.append("    }\n")
            }
        }
        ...
        val file = File(dir, "GeneratedStoreBindings.kt")
        val next = sb.toString()
        if (!file.isFile || file.readText() != next) {
            file.writeText(next)
            logger.lifecycle("syncForkConfig: regenerated GeneratedStoreBindings (${stores.size} store(s), ${purged.size} purged on logout)")
        }
    }
    ...
    private fun readLedgerMigrations(root: File): List<Triple<Int, Int, String?>> {
        val ledger = File(root, "app-profile/migration-ledger.yaml")
        if (!ledger.isFile) return emptyList()
        val rowRe = Regex("""from:\s*(\d+).*?to:\s*(\d+)""")
        val specRe = Regex("""spec:\s*([A-Za-z0-9_.]+)""")
        return ledger.readLines().mapNotNull { raw ->
            val line = raw.substringBefore('#')
            if (!line.trimStart().startsWith("-")) return@mapNotNull null
            val m = rowRe.find(line) ?: return@mapNotNull null
            Triple(m.groupValues[1].toInt(), m.groupValues[2].toInt(), specRe.find(line)?.groupValues?.get(1))
        }
    }

    /** Fork-owned effective schema version, from the ledger. */
    ...
    private fun regenerateDatabaseForkRegions(root: File, appProfile: Map<String, Any?>) {
        ...
        // Migrations come from the FORK-owned ledger, not app.yaml: app-profile declares WHAT the
        // template may add (units, no versions), the ledger records WHERE each landed in this fork's
        // sequence. Reading versions from app.yaml would put the template back in the fork's counter.
        val migrations = readLedgerMigrations(root)
        ...
        // Fork-owned effective version -> ForkDatabaseConfig, replacing TEMPLATE_BASE_VERSION+OFFSET.
        readLedgerVersion(root)?.let { v ->
            val cfg = File(root, "core/database/src/commonMain/kotlin/kpt/core/database/config/ForkDatabaseConfig.kt")
            if (cfg.isFile) {
                val txt = cfg.readText()
                val next = Regex("""const val VERSION\s*=\s*\d+""").replace(txt, "const val VERSION = $v")
                if (next != txt) {
                    cfg.writeText(next)
                    logger.lifecycle("syncForkConfig: ForkDatabaseConfig.VERSION = $v (from the ledger)")
                }
            }
        }

        // FRAMEWORK tables, declared by core-base (the module that owns them). Emitted into their own
        // regions so this file hardcodes nothing: adding a framework table is a declaration there
        // plus a migration unit, never a hand-edit of core/database.
        val infra = File(root, "core-base/database/module-schema.yaml")
        val infraEntities = mutableListOf<String>()
        val infraDaos = mutableListOf<Pair<String, String>>()
        if (infra.isFile) {
            var section = ""
            infra.readLines().forEach { raw ->
                val line = raw.substringBefore('#')
                when {
                    line.startsWith("entities:") -> section = "e"
                    line.startsWith("daos:") -> section = "d"
                    line.trimStart().startsWith("-") && section == "e" ->
                        fqn(line.substringAfter("-").trim())?.let { infraEntities += it }
                    line.trimStart().startsWith("-") && section == "d" -> {
                        val n = Regex("""name:\s*(\w+)""").find(line)?.groupValues?.get(1)
                        val t = Regex("""type:\s*([A-Za-z0-9_.]+)""").find(line)?.groupValues?.get(1)
                        if (n != null && t != null) infraDaos += n to t
                    }
                }
            }
        }
        val infraEntityBody = buildString {
            append("        // gen-infra-entities:begin\n")
            infraEntities.forEach { append("        ").append(it).append("::class,\n") }
            append("        // gen-infra-entities:end")
        }
        val infraDaoBody = buildString {
            append("    // gen-infra-daos:begin\n")
            infraDaos.forEach { (n, t) -> append("    abstract val ").append(n).append(": ").append(t).append('\n') }
            append("    // gen-infra-daos:end")
        }

        val before = file.readText()
        patchSentinel(file, "        // gen-infra-entities:begin", "        // gen-infra-entities:end", infraEntityBody)
        patchSentinel(file, "    // gen-infra-daos:begin", "    // gen-infra-daos:end", infraDaoBody)
        ...
        if (file.readText() != before) {
            logger.lifecycle(
                "syncForkConfig: regenerated AppDatabase fork regions " +
                    "(${infraEntities.size}+${entities.size} entities, ${infraDaos.size}+${daos.size} DAOs, ${migrations.size} migrations, ${converters.size} converters)",
            )
        }
    }
