# `fineract` — access point package

SCAFFOLDED by `./gradlew syncForkConfig` from the `fineract` access point in
...
- `api/` — the Ktorfit interface for this endpoint. Annotate it
  `@ApiBinding("fineract")` and its Koin binding is GENERATED into
  `di/GeneratedApiBindings.kt`; there is no wiring step.
- `dto/` — the wire types this endpoint returns.

`type: rest` — the binding is `restApi("fineract") { it.create${simple}Api() }`, so Ktorfit
